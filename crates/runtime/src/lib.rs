...
impl Runtime {
    ...
    pub fn evaluate_expression_with_context(
        ...
        match expr {
            ...
            Expression::FunctionCall { object, method, arguments } => {
                self.evaluate_function_call(object, method, arguments, context)
            },
            
            Expression::StringTemplate { parts } => {
                self.evaluate_string_template(parts, context)
            },
    ...
    fn evaluate_function_call(
        ...
        match (object, method) {
            ("", "len") => {
                ...
                let arg_value = self.evaluate_expression_with_context(&arguments[0].value, context)?;
                match arg_value {
                    JsonValue::String(s) => Ok(JsonValue::Number(serde_json::Number::from(s.len()))),
                    JsonValue::Array(arr) => Ok(JsonValue::Number(serde_json::Number::from(arr.len()))),
                    JsonValue::Object(obj) => Ok(JsonValue::Number(serde_json::Number::from(obj.len()))),
            ...
            ("", "abs") => {
                ...
                match arg_value {
                    JsonValue::Number(n) => {
                        let f = n.as_f64().ok_or_else(|| RuntimeError::Evaluation(
                            "Invalid number for abs()".to_string()
                        ))?;
                        Ok(JsonValue::Number(serde_json::Number::from_f64(f.abs())
            ...
            (plugin_name, method_name) if !plugin_name.is_empty() => {
                if let Some(plugin) = self.plugins.get(plugin_name) {
                    ...
                    for (i, arg) in arguments.iter().enumerate() {
                        ...
                        let arg_value = self.evaluate_expression_with_context(&arg.value, context)?;
    ...
    fn perform_arithmetic_operation(
        ...
        match (left, right) {
            (JsonValue::Number(l), JsonValue::Number(r)) => {
                let l_f64 = l.as_f64().ok_or_else(|| RuntimeError::Evaluation(
                    "Invalid left operand for arithmetic operation".to_string()
                ))?;
                let r_f64 = r.as_f64().ok_or_else(|| RuntimeError::Evaluation(
                ...
                Ok(JsonValue::Number(serde_json::Number::from_f64(result)
                    .ok_or_else(|| RuntimeError::Evaluation(
            ...
            _ => Err(RuntimeError::Evaluation(
                format!("Cannot perform arithmetic operation on {:?} and {:?}", left, right)
            )),
        }
    }
    ...
    fn perform_comparison(
        ...
        match (left, right) {
            (JsonValue::Number(l), JsonValue::Number(r)) => {
                let l_f64 = l.as_f64().ok_or_else(|| RuntimeError::Evaluation(
                    "Invalid left operand for comparison".to_string()
                ))?;
                let r_f64 = r.as_f64().ok_or_else(|| RuntimeError::Evaluation(
                    "Invalid right operand for comparison".to_string()
                ))?;
        ...
        match (left, right) {
            ...
            (JsonValue::Object(l), JsonValue::Object(r)) => {
                l.len() == r.len() && l.iter().all(|(k, v)| {
                    r.get(k).map_or(false, |rv| self.values_equal(v, rv))
                })
            },
            _ => false,
        }
    ...
    fn is_truthy(&self, value: &JsonValue) -> bool {
        match value {
            ...
            JsonValue::Bool(b) => *b,
            JsonValue::Number(n) => n.as_f64().map_or(false, |f| f != 0.0),
    ...
    fn perform_array_access(
        ...
        match (array, index) {
            (JsonValue::Array(arr), JsonValue::Number(n)) => {
                let idx = n.as_u64().ok_or_else(|| RuntimeError::Evaluation(
    ...
    fn perform_property_access(
        ...
        match object {
            JsonValue::Object(obj) => {
                Ok(obj.get(property).cloned().unwrap_or(JsonValue::Null))
            },
            _ => Err(RuntimeError::Evaluation(
...
mod tests {
    ...
    fn test_enhanced_arithmetic_expressions() {
        ...
        assert_eq!(result, serde_json::Value::Number(serde_json::Number::from_f64(8.0).unwrap()));
        ...
        assert_eq!(result, serde_json::Value::Number(serde_json::Number::from_f64(6.0).unwrap()));
        ...
        assert_eq!(result, serde_json::Value::Number(serde_json::Number::from_f64(12.0).unwrap()));
        ...
        assert_eq!(result, serde_json::Value::Number(serde_json::Number::from_f64(5.0).unwrap()));
        ...
        assert_eq!(result, serde_json::Value::Number(serde_json::Number::from_f64(2.0).unwrap()));
    }
    ...
    fn test_enhanced_builtin_functions() {
        ...
        assert_eq!(result, serde_json::Value::Number(serde_json::Number::from_f64(5.5).unwrap()));
    }
    ...
    fn test_enhanced_string_templates() {
        ...
        context.insert("age".to_string(), serde_json::Value::Number(serde_json::Number::from(30)));
        ...
        let result = runtime.evaluate_expression_with_context(&template_expr, &context).unwrap();
        assert_eq!(result, serde_json::Value::String("Hello, Alice! You are 30 years old.".to_string()));
        ...
        let result = runtime.evaluate_expression_with_context(&template_missing_expr, &context).unwrap();
        assert_eq!(result, serde_json::Value::String("Hello, ${unknown}!".to_string()));
    }
    ...
    fn test_enhanced_variable_resolution() {
        let runtime = Runtime::new();
        let mut context = std::collections::HashMap::new();
        context.insert("x".to_string(), serde_json::Value::Number(serde_json::Number::from(42)));
        ...
        let result = runtime.evaluate_expression_with_context(&var_expr, &context).unwrap();
        ...
        let result = runtime.evaluate_expression_with_context(&str_var_expr, &context).unwrap();
        assert_eq!(result, serde_json::Value::String("Hello".to_string()));
        
        // Test unknown variable (returns placeholder)
        let unknown_expr = Expression::Identifier("unknown".to_string());
        let result = runtime.evaluate_expression_with_context(&unknown_expr, &context).unwrap();
    ...
    fn test_enhanced_complex_expressions() {
        let runtime = Runtime::new();
        let mut context = std::collections::HashMap::new();
        context.insert("a".to_string(), serde_json::Value::Number(serde_json::Number::from(10)));
        context.insert("b".to_string(), serde_json::Value::Number(serde_json::Number::from(5)));
        ...
        let result = runtime.evaluate_expression_with_context(&complex_expr, &context).unwrap();
        assert_eq!(result, serde_json::Value::Number(serde_json::Number::from_f64(30.0).unwrap()));
        ...
        let result = runtime.evaluate_expression_with_context(&complex_cond, &context).unwrap();
