...
impl SigmosParser {
        ...
        while let Some((i, ch)) = chars.next() {
            match ch {
                ...
                '"' => {
                    ...
                    if !found_end {
                        return Err(ParseError::Grammar("Unterminated string literal".to_string()));
                    }
    ...
    fn parse_version(version_str: &str) -> Result<(u32, u32, Option<u32>), Box<dyn std::error::Error>> {
    ...
    fn parse_specification(&mut self) -> ParseResult<Spec> {
        ...
        let name = match self.advance() {
            ...
            _ => return Err(ParseError::Grammar("Expected spec name as string literal".to_string())),
        };
        
        let version = match self.advance() {
            Token::Version(major, minor, patch) => Version { major, minor, patch },
            _ => return Err(ParseError::Grammar("Expected version (e.g., v1.0)".to_string())),
        };
            ...
            match self.peek() {
                Token::Description => {
                    ...
                    if let Token::StringLiteral(desc) = self.advance() {
                        ...
                        return Err(ParseError::Grammar("Expected string literal for description".to_string()));
                    }
                }
                Token::Inputs => {
                    self.advance();
    ...
    fn expect_token(&mut self, expected: Token) -> ParseResult<()> {
        if self.check(&expected) {
            ...
            Err(ParseError::Grammar(format!("Expected {:?}, found {:?}", expected, self.peek())))
        }
    }
}
