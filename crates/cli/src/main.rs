...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Validate { file } => validate_spec(&file).await?,
        Commands::Run { file, config } => run_spec(&file, config.as_ref()).await?,
        Commands::Transpile { file, to, output } => {
            transpile_spec(&file, to, output.as_ref()).await?
        }
        Commands::Install { plugin } => install_plugin(&plugin).await?,
        Commands::Plugin { command } => match command {
            PluginCommands::New { name } => create_plugin_scaffold(&name).await?,
        },
        Commands::Describe { file } => describe_spec(&file).await?,
    }
...
async fn transpile_spec(
    ...
    let result = match format {
        OutputFormat::Json => transpiler
            .to_json(&spec)
            .map_err(|e| miette::miette!("Transpiler error: {}", e))?,
        OutputFormat::Yaml => transpiler
            .to_yaml(&spec)
            .map_err(|e| miette::miette!("Transpiler error: {}", e))?,
        OutputFormat::Toml => transpiler
            .to_toml(&spec)
            .map_err(|e| miette::miette!("Transpiler error: {}", e))?,
    };
...
async fn install_local_plugin(plugin_path: &std::path::Path) -> Result<()> {
    ...
    if !cargo_toml.exists() {
        return Err(miette::miette!(
            "Invalid plugin: Cargo.toml not found in {}",
            plugin_path.display()
        ));
    }
    ...
    if !cargo_content.contains("sigmos-core") {
        return Err(miette::miette!(
            "Invalid SIGMOS plugin: missing sigmos-core dependency"
        ));
    }
...
async fn create_plugin_scaffold(name: &str) -> Result<()> {
    ...
    {
        return Err(miette::miette!(
            "Invalid plugin name. Use only alphanumeric characters, hyphens, and underscores."
        ));
    }
...
[dependencies]
sigmos-plugins = {{ path = "../plugins" }}
sigmos-runtime = {{ path = "../runtime" }}
