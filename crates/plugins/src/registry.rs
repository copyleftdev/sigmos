...
impl PluginRegistry {
    ...
    pub fn execute_plugin_method(
        ...
        let entry = self.get_plugin(plugin_name).ok_or_else(|| {
            sigmos_runtime::RuntimeError::Plugin(format!("Plugin '{plugin_name}' not found"))
        })?;
        ...
        for (name, entry) in &self.plugins {
            if entry.enabled {
                if let Ok(mut plugin) = entry.plugin.write() {
                    ...
                    failed_plugins.push(format!("{name}: Failed to acquire write lock"));
                }
            }
        }
    ...
    pub fn get_plugins_by_capability(
        &self,
        check: impl Fn(&PluginCapabilities) -> bool,
    ) -> Vec<String> {
        self.plugins
            .iter()
...
mod tests {
    ...
    fn test_plugin_registry_basic() {
        ...
        assert!(registry
            .register_plugin(plugin, metadata, capabilities)
            .is_ok());
    ...
    fn test_plugin_registry_aliases() {
        ...
        assert!(registry
            .register_plugin_with_aliases(plugin, metadata, capabilities, aliases)
            .is_ok());
