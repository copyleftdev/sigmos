    ...
    fn execute(&self, method: &str, args: &HashMap<String, JsonValue>) -> RuntimeResult<JsonValue> {
        if !self.initialized {
            return Err(RuntimeError::Plugin(
                "MCP plugin not initialized".to_string(),
            ));
        }
...
impl McpPlugin {
    ...
    fn complete(&self, args: &HashMap<String, JsonValue>) -> RuntimeResult<JsonValue> {
        ...
        Ok(JsonValue::Object({
            ...
            obj.insert(
                "model".to_string(),
                JsonValue::String(self.config.model.clone()),
            );
            obj.insert(
        ...
        Ok(JsonValue::Object({
            ...
            obj.insert(
                ...
                JsonValue::Array(
                    embedding
                        .into_iter()
                        .map(|f| JsonValue::Number(serde_json::Number::from_f64(f as f64).unwrap()))
                        .collect(),
                ),
            );
            obj.insert(
                "dimensions".to_string(),
                JsonValue::Number(serde_json::Number::from(768)),
            );
            obj.insert(
    ...
    fn chat(&self, args: &HashMap<String, JsonValue>) -> RuntimeResult<JsonValue> {
        ...
        Ok(JsonValue::Object({
            ...
            obj.insert(
                "response".to_string(),
                JsonValue::String("MCP chat response".to_string()),
            );
            obj.insert(
                "role".to_string(),
    ...
    fn analyze(&self, args: &HashMap<String, JsonValue>) -> RuntimeResult<JsonValue> {
        ...
        Ok(JsonValue::Object({
            ...
            obj.insert(
                "sentiment".to_string(),
                JsonValue::String("positive".to_string()),
            );
            obj.insert(
                "confidence".to_string(),
                JsonValue::Number(serde_json::Number::from_f64(0.85).unwrap()),
            );
            obj.insert(
                "word_count".to_string(),
                JsonValue::Number(serde_json::Number::from(text.split_whitespace().count())),
            );
